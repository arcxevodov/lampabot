...
bot.command('rating', async (ctx) => {
    ...
    rating.forEach((key, value) => {
        result += `${value+1}. ${key['full_name'] + ' (' + key['username'] ?? 'без ника' + ')' ?? '@' + key['username']}, от тебя ангел улетел на ${key['count']} ${BotFunctions.getNumEnding(key['count'], ['метр', 'метра', 'метров'])}\n\n`
    })
